...
#    puts "w = " + w
    degree_block = 'N' + n + 'W' + w
#    puts "processing degree_block = " + degree_block
  end    
...
#  puts x3
  ...
  %w{ DSM DTM ORI }.each do |d|
    Dir.glob( d + "/*FUGRO*").each do | t | # TODO correct the directory inside the ()
#      puts "t = " + t
      if (degree_block == t.split('_').last) && (x2 == d)
        system "cp -rlv " + p + " " + t + "/orig_geocells/"
#        puts "cp -rlv " + p + " " + t + "/orig_geocells/"
      end
    end
